...
export function DigitalWorkflow() {
  const steps = [
    {
      ...
      icon: <Scan className="h-6 w-6 text-[#246B5B]" />,
    },
    {
      num: "02",
      ...
      icon: <Eye className="h-6 w-6 text-[#246B5B]" />,
    },
    {
      num: "03",
      ...
      icon: <Activity className="h-6 w-6 text-[#246B5B]" />,
    },
  ];
  ...
  return (
    <section id="technology" className="relative py-24 sm:py-32 bg-[#FFFFFF] border-t border-black/5 overflow-hidden">
      {/* Background Accent */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-[#DCEDE7]/30 rounded-full blur-3xl pointer-events-none -z-10" />
      ...
      <div className="max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
          ...
          <div className="hidden lg:block absolute top-12 inset-x-12 h-px bg-gradient-to-r from-[#246B5B]/20 via-[#246B5B]/40 to-[#246B5B]/20 -z-10" />

          {steps.map((step, idx) => (
            <div
              key={step.num}
              className="group relative flex flex-col justify-between rounded-2xl bg-[#F7F8F6] p-8 sm:p-10 border border-black/5 hover:border-[#246B5B]/30 hover:shadow-lg transition-all duration-300"
            >
              <div>
                {/* Step Top Header */}
                <div className="flex items-center justify-between mb-8">
                  <span className="font-mono text-3xl sm:text-4xl font-light text-[#246B5B]">
                    {step.num}
                  </span>
                ...
                <div className="text-[11px] font-mono tracking-wider uppercase text-[#246B5B] mb-1.5 font-semibold">
                  {step.tag}
                </div>

                <h3 className="text-2xl font-normal font-serif text-[#111816] tracking-tight mb-4">
                  {step.title}
                </h3>

                <p className="text-sm sm:text-base text-[#68716D] leading-relaxed font-light mb-8">
                  {step.desc}
                </p>
              </div>
              ...
              <div className="pt-6 border-t border-black/8 space-y-2">
                {step.specs.map((spec, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs text-[#111816]/80">
