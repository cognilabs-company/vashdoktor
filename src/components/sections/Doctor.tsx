...
export function Doctor({ onOpenConsultation }: DoctorProps) {
  return (
    <section id="specialist" className="py-28 sm:py-36 bg-[var(--c-bg)] border-t border-white/10 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div data-reveal="left" className="lg:col-span-6 relative">
            <div className="relative aspect-[4/5] w-full rounded-3xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.35)] border border-white/10 bg-[#0d2230]">
              ...
              <div className="absolute bottom-6 left-6 right-6 p-4 rounded-2xl bg-[var(--c-bg)]/70 backdrop-blur-md border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs font-semibold text-white">Dr. Alexander Sinclair</div>
                    <div className="text-[11px] text-[var(--c-text-4)]">DDS, PhD • Yetakchi implantolog</div>
                  </div>
                  <div className="flex items-center gap-1 text-[var(--c-accent)] text-xs font-mono">
          ...
          <div data-stagger className="lg:col-span-6 flex flex-col items-start">
            ...
            <h2 className="mt-6 text-3xl sm:text-4xl lg:text-[2.5rem] font-medium font-serif tracking-tight text-white leading-[1.08]">
              ...
              <span className="font-medium text-[var(--c-accent)]">Tajriba hal qiladi.</span>
            </h2>

            {/* Blockquote */}
            <div className="mt-8 relative pl-6 border-l-2 border-[var(--c-accent)]/40 italic text-base sm:text-lg text-[var(--c-text-2)] font-serif">
              <Quote className="absolute -top-3 -left-3 h-6 w-6 text-[var(--c-accent)]/20 pointer-events-none" />
              "{LEAD_SURGEON.quote}"
            </div>

            <p className="mt-6 text-sm sm:text-base text-[var(--c-text-2)] font-light leading-relaxed">
            ...
            <div className="mt-8 grid grid-cols-3 gap-4 w-full py-6 border-y border-white/10">
              {LEAD_SURGEON.stats.map((stat, i) => (
                <div key={i} className="text-left">
                  <div className="text-2xl sm:text-3xl font-serif text-[var(--c-accent)] tabular-nums">
                    <CountUp value={stat.value} />
                  </div>
                  <div className="text-[11px] font-mono text-[var(--c-text-4)] uppercase mt-1">
                    {stat.label}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-6 space-y-2.5 w-full">
              {LEAD_SURGEON.credentials.map((cred, i) => (
                <div key={i} className="flex items-start gap-2.5 text-xs text-[var(--c-text-2)]">
                  <Award className="h-4 w-4 text-[var(--c-accent)] shrink-0 mt-0.5" />
                  <span>{cred}</span>
                </div>
              ))}
            </div>
