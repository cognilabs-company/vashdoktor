...
function ServiceCard({ s, detailed }: { s: Service; detailed?: boolean }) {
  ...
  return (
    <article className="group flex flex-col rounded-2xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-[#8fc7d4]/40 hover:bg-white/[0.06] hover:shadow-[0_20px_50px_-30px_rgba(143,199,212,0.5)]">
      ...
      <p className="mt-2.5 text-sm leading-relaxed text-[#a7c2cb]">{detailed ? s.body : s.short}</p>
      {detailed && (
...
export function ServicesSection({ services, eyebrow = 'Xizmatlar', title, subtitle, cta, detailed, bg = '#0a141d' }: Props) {
  return (
                ...
                <div key={cat.key}>
                  <div className="flex flex-wrap items-end justify-between gap-3 border-b border-white/10 pb-5">
                    <div data-reveal="left">
                      <span className="font-mono text-[11px] tracking-[0.3em] text-[#5c7580]">
                    ...
                    <p className="max-w-sm text-sm leading-relaxed text-[#8fb0ba]" data-reveal="right">{cat.note}</p>
                  </div>
